	...
	if(false !== $idr)
	{
		if($action == "add") {
			...
			if($idr!=0){ 
				...
				if (creationPanier())
				{ 
				  $vprice = isset($_GET['vprice']) ? $_GET['vprice'] : false;
				  $vname = isset($_GET['vname']) ? urldecode($_GET['vname']) : false;
				  
				  $positionProduit = false;
				  for($i=0; $i < count($_SESSION['panier']['idcart']); $i++){
					  $sess_var = isset($_SESSION['panier']['variation'][$i]) ? $_SESSION['panier']['variation'][$i] : false;
					  if($_SESSION['panier']['idcart'][$i] == $idr && $sess_var === $vname) {
						  $positionProduit = $i;
						  break;
					  }
				  }
					...
					{ 
						 ...
						 if($_SESSION['panier']['promo'][$positionProduit] > 0) 
					...
					else
					{
						  ...
						  if($vprice !== false && $vprice > 0) {
							  // If variation has a specific price, use it
						   ...
						   if($promo > 0) $total_ligne = $promo*$qte_prd; else $total_ligne = $price*$qte_prd;
						 ...
						 if(!isset($_SESSION['panier']['variation'])) $_SESSION['panier']['variation'] = array();
						 array_push( $_SESSION['panier']['variation'], $vname);
					}
